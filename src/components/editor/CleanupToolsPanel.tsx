...
/**
 * إصلاح الألف المحذوفة بعد ال التعريف
 * مثال: النتظار → الانتظار، الستخدام → الاستخدام
 * 
 * القاعدة: في العربية، كلمات كثيرة تبدأ بهمزة وصل (ا) مثل:
 * انتظار، استخدام، اختيار، انطلاق، استعداد، اقتراب، انتهاء...
 * عند إضافة "ال" تصبح: الانتظار، الاستخدام...
 * الذكاء الاصطناعي أحياناً يحذف هذه الألف: النتظار، الستخدام
 */
function fixMissingAlef(text: string): string {
  // Pattern: "ال" followed by consonant clusters that indicate a missing alef
  // These are common Arabic verb/noun prefixes that require an initial alef:
  // نت، ست، خت، نط، نف، نق، نش، نح، نس، نك، ند، نذ، نب، نج، نص، نض، نظ، نع، نغ، نم، نو، نه، ني
  // ست، شت، حت، فت، كت، بت، جت، صت، ضت، طت، ظت، عت، غت، هت، وت، يت
  // Plus common patterns like: لت، قت، رت
  
  // Common prefixes of Arabic words that start with hamzat wasl (اِ):
  // اِنْ، اِسْ، اِخْ، اِفْ، اِقْ، اِجْ، اِعْ، اِتّ، اِحْ، اِكْ، اِلْ، اِمْ، اِنْ
  
  // We look for "ال" + two consonants where the first is common (ن، س، خ، ف، ق، ج، ع، ت، ح، ك، ل، م، ر، ب، ش، ص، ض، ط، ظ، غ)
  // and the combo should be a known pattern
  
  const ARABIC_CONSONANTS = 'بتثجحخدذرزسشصضطظعغفقكلمنهوي';
  
  // Known prefixes that commonly appear after "ال" and need an initial alef
  // These represent the first 2 consonants of words starting with hamzat wasl
  const KNOWN_MISSING_ALEF_PATTERNS = new Set([
    // اِنْتـ، اِنْطـ، اِنْفـ، اِنْقـ، اِنْشـ، اِنْحـ، اِنْسـ، اِنْكـ، اِنْدـ، اِنْذـ، اِنْبـ، اِنْجـ، اِنْصـ، اِنْضـ، اِنْظـ، اِنْعـ، اِنْغـ، اِنْمـ، اِنْوـ، اِنْهـ
    'نت', 'نط', 'نف', 'نق', 'نش', 'نح', 'نس', 'نك', 'ند', 'نذ', 'نب', 'نج', 'نص', 'نض', 'نظ', 'نع', 'نغ', 'نم', 'نو', 'نه',
    // اِسْتـ، اِسْمـ، اِسْنـ
    'ست', 'سم', 'سن', 'سر', 'سل',
    // اِخْتـ، اِخْفـ، اِخْرـ
    'خت', 'خف', 'خر', 'خل',
    // اِفْتـ، اِفْرـ
    'فت', 'فر',
    // اِقْتـ، اِقْرـ، اِقْصـ
    'قت', 'قر', 'قص', 'قل',
    // اِجْتـ، اِجْرـ
    'جت', 'جر', 'جم',
    // اِعْتـ، اِعْمـ، اِعْرـ، اِعْصـ
    'عت', 'عم', 'عر', 'عص', 'عل', 'عد',
    // اِتّـ
    ...
    // اِرْتـ
    'رت', 'رم', 'رف',
    // رسـ common in specific words
    'رس',
  ]);
  
  // Match "ال" + two Arabic consonant letters (without alef between them)
