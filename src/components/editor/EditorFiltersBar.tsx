      ...
      {isMobile ? (
        ...
        <>
          <select value={editor.filterStatus} onChange={e => editor.setFilterStatus(e.target.value as FilterStatus)} className="px-3 py-2 rounded bg-background border border-border font-body text-sm">
            ...
            <option value="uppercase">🔠 أحرف إنجليزية كبيرة</option>
            ...
            {editor.deepDiagnosticCounts.breakMissing > 0 && <option value="break-missing">▼ ينقصه فاصل توقّف ({editor.deepDiagnosticCounts.breakMissing})</option>}
