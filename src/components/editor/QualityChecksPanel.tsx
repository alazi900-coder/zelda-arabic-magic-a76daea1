...
function checkGrammar(translation: string): { type: string; message: string; fix?: string }[] {
  ...
  const rules: [RegExp, string, RegExp?, string?][] = [
    [/الال/, 'تعريف مكرر "الال"', /الال/g, 'ال'],
    [/ةة/, 'تاء مربوطة مكررة "ةة"', /ةة/g, 'ة'],
    // همزة القطع بدل الوصل
    [/\bإستخدام/, 'همزة خاطئة: "إستخدام" ← "استخدام"', /إستخدام/g, 'استخدام'],
    [/\bإستطاع/, 'همزة خاطئة: "إستطاع" ← "استطاع"', /إستطاع/g, 'استطاع'],
    [/\bإنتقل/, 'همزة خاطئة: "إنتقل" ← "انتقل"', /إنتقل/g, 'انتقل'],
    [/\bإنتظ/, 'همزة خاطئة: "إنتظر/إنتظار" ← "انتظر/انتظار"', /إنتظ/g, 'انتظ'],
    [/\bإستمر/, 'همزة خاطئة: "إستمر" ← "استمر"', /إستمر/g, 'استمر'],
    [/\bإكتشف/, 'همزة خاطئة: "إكتشف" ← "اكتشف"', /إكتشف/g, 'اكتشف'],
    [/\bإنطلق/, 'همزة خاطئة: "إنطلق" ← "انطلق"', /إنطلق/g, 'انطلق'],
    [/\bإحتاج/, 'همزة خاطئة: "إحتاج" ← "احتاج"', /إحتاج/g, 'احتاج'],
    [/\bإختر\b/, 'همزة خاطئة: "إختر" ← "اختر"', /\bإختر\b/g, 'اختر'],
    [/\bإختيار/, 'همزة خاطئة: "إختيار" ← "اختيار"', /إختيار/g, 'اختيار'],
    [/\bإنضم/, 'همزة خاطئة: "إنضم" ← "انضم"', /إنضم/g, 'انضم'],
    [/\bإستعد/, 'همزة خاطئة: "إستعد" ← "استعد"', /إستعد/g, 'استعد'],
    [/\bإستلم/, 'همزة خاطئة: "إستلم" ← "استلم"', /إستلم/g, 'استلم'],
    // ألف مقصورة vs ياء
    [/\bالي\b/, 'ألف مقصورة: "الي" ← "إلى"', /\bالي\b/g, 'إلى'],
    [/\bعلي\b(?!\s+(?:بن|ابن|أن))/, 'ألف مقصورة: "علي" ← "على"', /\bعلي\b(?!\s+(?:بن|ابن|أن))/g, 'على'],
    [/\bحتي\b/, 'ألف مقصورة: "حتي" ← "حتى"', /\bحتي\b/g, 'حتى'],
    [/\bمتي\b/, 'ألف مقصورة: "متي" ← "متى"', /\bمتي\b/g, 'متى'],
    [/\bلدي\b/, 'ألف مقصورة: "لدي" ← "لدى"', /\bلدي\b/g, 'لدى'],
    [/\bسوي\b/, 'ألف مقصورة: "سوي" ← "سوى"', /\bسوي\b/g, 'سوى'],
    [/\bأخري\b/, 'ألف مقصورة: "أخري" ← "أخرى"', /\bأخري\b/g, 'أخرى'],
    [/\bكبري\b/, 'ألف مقصورة: "كبري" ← "كبرى"', /\bكبري\b/g, 'كبرى'],
    [/\bصغري\b/, 'ألف مقصورة: "صغري" ← "صغرى"', /\bصغري\b/g, 'صغرى'],
    // تاء مربوطة vs هاء
    [/\bهذة\b/, 'تاء مربوطة: "هذة" ← "هذه"', /\bهذة\b/g, 'هذه'],
    [/\bمفتوحت\b/, 'تاء خاطئة: "مفتوحت" ← "مفتوحة"', /\bمفتوحت\b/g, 'مفتوحة'],
    [/\bمغلقت\b/, 'تاء خاطئة: "مغلقت" ← "مغلقة"', /\bمغلقت\b/g, 'مغلقة'],
    // ال الشمسية مع مسافة
    [/ال\s+[تثدذرزسشصضطظنل]/, 'مسافة بعد ال التعريف قبل حرف شمسي', /ال\s+([تثدذرزسشصضطظنل])/g, 'ال$1'],
    // حرف مكرر 3+
    [/(.)\1{2,}/, 'حرف مكرر أكثر من مرتين'],
    // نقطتان متتاليتان
    [/\.{2}(?!\.)/, 'نقطتان متتاليتان', /\.{2}(?!\.)/g, '.'],
    // فاصلة إنجليزية بدل عربية
    [/[\u0600-\u06FF],[\u0600-\u06FF]/, 'فاصلة إنجليزية بين كلمات عربية', /([\u0600-\u06FF]),([\u0600-\u06FF])/g, '$1،$2'],
    // علامة استفهام إنجليزية
    [/[\u0600-\u06FF]\s*\?/, 'علامة استفهام إنجليزية مع نص عربي', /\?/g, '؟'],
    // فاصلة منقوطة إنجليزية
    [/[\u0600-\u06FF]\s*;/, 'فاصلة منقوطة إنجليزية', /;/g, '؛'],
    // إملاء
    [/\bالاعب\b/, 'إملاء: "الاعب" ← "اللاعب"', /\bالاعب\b/g, 'اللاعب'],
    [/\s[،؟؛!]/, 'مسافة زائدة قبل علامة ترقيم', /\s([،؟؛!])/g, '$1'],
    [/شيئ\b/, 'إملاء: "شيئ" ← "شيء"', /شيئ\b/g, 'شيء'],
    [/مسئول/, 'إملاء: "مسئول" ← "مسؤول"', /مسئول/g, 'مسؤول'],
    [/إنشاء\s*الله/, 'إملاء: "إنشاء الله" ← "إن شاء الله"', /إنشاء\s*الله/g, 'إن شاء الله'],
...
export default function QualityChecksPanel({ state, onApplyFix, onFilterByKeys, onNavigateToEntry, glossary, risenVariant }: QualityChecksPanelProps) {
  ...
  const handleContextCheck = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('translation-tools', {
        body: { style: 'context-check', entries, glossary: glossary?.slice(0, 3000), game: gameParam },
      });
      if (error) throw error;
      if (data?.result) {
        try {
  ...
  const handleBatchImprove = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('translation-tools', {
        body: { style: 'batch-improve', entries, improvementStyle, glossary: glossary?.slice(0, 3000), game: gameParam },
      });
      if (error) throw error;
      if (data?.result) {
        try {
