      ...
      >
        ...
        <div className="overflow-y-auto flex-1 py-1">
          ...
          {filtered.map((node) => {
            ...
            const allTablesInCat = [...node.subs.values()].flat();
            return (
              <div key={node.label} className="border-t border-border/50">
                <Collapsible open={catOpen} onOpenChange={() => toggleCat(node.label)}>
                  <div className="flex items-center hover:bg-accent/60 transition-colors">
                    <CollapsibleTrigger className="flex-1 flex items-center justify-between px-3 py-2 text-sm font-display font-semibold">
                      ...
                      <span className="text-[10px] opacity-60 font-body">
                        {allTablesInCat.length} جدول
                      </span>
                    </CollapsibleTrigger>
                    {onDeleteScope && (
                      <button
                        onClick={(e) => { e.stopPropagation(); onDeleteScope(allTablesInCat, node.label); }}
                        className="px-2 py-2 text-destructive hover:bg-destructive/10 transition-colors"
                        title={`حذف كل ترجمات قسم ${node.label}`}
                      >
                    ...
                    {[...node.subs.entries()].map(([sk, arr]) => {
                      ...
                      return (
                        <Collapsible key={subKey} open={subOpen} onOpenChange={() => toggleSub(subKey)}>
                          <div className="flex items-center hover:bg-accent/40 transition-colors">
                            <CollapsibleTrigger className="flex-1 flex items-center justify-between pr-6 pl-3 py-1.5 text-xs font-body text-muted-foreground">
                            ...
                            {onDeleteScope && (
                              <button
                                onClick={(e) => { e.stopPropagation(); onDeleteScope(arr, sk); }}
                                className="px-2 py-1.5 text-destructive hover:bg-destructive/10 transition-colors"
                                title={`حذف ترجمات فرع ${sk}`}
                              >
                          ...
                          <CollapsibleContent>
                            {arr.map((t) => (
                              <div key={t} className="flex items-center hover:bg-accent transition-colors">
                                <button
                                  onClick={() => pick(t)}
                                  className={cn(
                                    "flex-1 text-right pr-10 pl-3 py-1.5 text-xs font-body flex items-center justify-between gap-2",
                                ...
                                {onDeleteScope && (
                                  <button
                                    onClick={(e) => { e.stopPropagation(); onDeleteScope([t], t); }}
                                    className="px-2 py-1.5 text-destructive hover:bg-destructive/10 transition-colors"
                                    title={`حذف ترجمات جدول ${t}`}
                                  >
                                    <Trash2 className="w-3 h-3" />
                                  </button>
                                )}
