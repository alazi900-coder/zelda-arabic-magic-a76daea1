...
describe("Pokémon rules in the AI enhancement tool", () => {
  ...
  it("withholds Xenoblade's tag rules from Pokémon and LumenTale", () => {
    ...
    expect(EDGE_SOURCE).toContain("(!XENOBLADE_TAG_RULE_IDS.has(r.id) || (!isPokemon && !isLumenTale && !isGtaIv && !isPlatinum && !isCrashlands && !isNinthDawn && !isInazuma && !isFranBow))");
  });
...
describe("Crashlands rules in the AI enhancement tool", () => {
  ...
  it("is injected only for Crashlands", () => {
    ...
    expect(EDGE_SOURCE).toContain("isPlatinum, isCrashlands, isNinthDawn, isInazuma, isFranBow)");
  });
