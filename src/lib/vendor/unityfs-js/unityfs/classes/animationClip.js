...
import { Quaternion } from '../basicTypes.js'
import { NamedObject } from './namedObject.js'
import { getClassName } from '../utils.js'

export class Keyframe {
    static exposedAttributes = ['time', 'value', 'inSlope', 'outSlope']
...
export class AnimationCurve {
    static exposedAttributes = ['curve', 'preInfinity', 'postInfinity', 'rotationOrder']
...
export class QuaternionCurve {
    static exposedAttributes = ['curve', 'path']
...
export class PackedFloatVector {
    static exposedAttributes = ['length', 'range', 'start', 'bitSize']
    ...
    unpack(chunkItemCount, chunkStride, start = 0, numChunks = -1) {
        ...
        const scale = 1 / range
        const dataScale = scale * ((1 << bitSize) - 1)

        if (numChunks === -1) {
            numChunks = Math.floor(this.length / chunkItemCount)
        }
        const end = (chunkStride * numChunks) / 4
        const data = new Float32Array(numChunks * chunkItemCount)
        let dataIdx = 0
        const u8Data = this.data
        ...
        for (let index = 0; index < end; index += chunkStride / 4) {
            for (let i = 0; i < chunkItemCount; i++) {
                ...
                let bits = 0
                while (bits < bitSize) {
                    x |= (u8Data[indexPos] >> bitPos) << bits
                    const bitsLeft = bitSize - bits
                    const byteLeft = 8 - bitPos
                    const num = bitsLeft < byteLeft ? bitsLeft : byteLeft
                    bitPos += num
                    bits += num
                    if (bitPos >= 8) {
                        indexPos++
                ...
                x &= (1 << bitSize) - 1
                data[dataIdx++] = x / dataScale + startVal
            }
        }
        return data
    }
...
export class PackedIntVector {
    static exposedAttributes = ['length', 'bitSize']
...
export class PackedQuaternionVector {
    ...
    unpack() {
        ...
        for (let i = 0; i < length; i++) {
            ...
            let bits = 0
            while (bits < 3) {
                flags |= (u8Data[indexPos] >> bitPos) << bits
                const bitsLeft = 3 - bits
                const byteLeft = 8 - bitPos
                const num = bitsLeft < byteLeft ? bitsLeft : byteLeft
                bitPos += num
                bits += num
            ...
            let sum = 0
            const flagIndex = flags & 3

            for (let j = 0; j < 4; j++) {
                if (j !== flagIndex) {
                    const bitSize = j === (flagIndex + 1) % 4 ? 9 : 10
                    let x = 0
                    let bits = 0
                    while (bits < bitSize) {
                        x |= (u8Data[indexPos] >> bitPos) << bits
                    ...
                    x &= (1 << bitSize) - 1
                    const val = x / (0.5 * ((1 << bitSize) - 1)) - 1
                    quat.setIndex(j, val)
                    sum += val * val
                }
            }

            const lastComponent = flagIndex
            const sqrVal = 1 - sum
            const lastVal = sqrVal > 0 ? Math.sqrt(sqrVal) : 0
            quat.setIndex(lastComponent, (flags & 4) !== 0 ? -lastVal : lastVal)
            data[i] = quat
        }
        return data
    }
...
export class CompressedAnimationCurve {
    static exposedAttributes = ['path', 'times', 'values', 'slopes', 'preInfinity', 'postInfinity']
...
export class FloatCurve {
    static exposedAttributes = ['curve', 'attribute', 'path', 'classID', 'script']
...
export class PPtrKeyframe {
    static exposedAttributes = ['time', 'value']
...
export class AABB {
    static exposedAttributes = ['center', 'extent']
...
export class AnimTransform {
    static exposedAttributes = ['t', 'q', 's']
...
export class HandPose {
    static exposedAttributes = ['grabX', 'doFArray', 'override', 'closeOpen', 'inOut', 'grab']
...
export class HumanGoal {
    static exposedAttributes = ['x', 'weightT', 'weightR', 'hintT', 'hintWeightT']
...
export class StreamedCurveKey {
    static exposedAttributes = ['index', 'coefficient', 'outSlope', 'value']
...
export class StreamedFrame {
    static exposedAttributes = ['time', 'keys']
...
export class StreamedClip {
    static exposedAttributes = ['curveCount']
        ...
        for (let frameIndex = 2; frameIndex < frameList.length - 1; frameIndex++) {
            ...
            for (let curveKey of frame.keys) {
                for (let i = frameIndex - 1; i >= 0; i--) {
                    ...
                    let preCurveKey = preFrame.keys.find((x) => x.index === curveKey.index)
...
export class DenseClip {
    static exposedAttributes = ['frameCount', 'curveCount', 'sampleRate', 'beginTime', 'samples']
...
export class ValueConstant {
    static exposedAttributes = ['id', 'type', 'index']
...
export class ValueArrayConstant {
    static exposedAttributes = ['values']
...
export class Clip {
    static exposedAttributes = ['streamedClip', 'denseClip', 'constantClip']
...
export class ValueDelta {
    static exposedAttributes = ['start', 'stop']
...
export class GenericBinding {
    static exposedAttributes = ['path', 'attribute', 'script', 'typeID', 'customType', 'isPPtrCurve', 'isIntCurve']
...
export class AnimationClipBindingConstant {
    static exposedAttributes = ['genericBindings', 'pptrCurveMapping']
