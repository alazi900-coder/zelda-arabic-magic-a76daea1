import { ValueArrayConstant } from './animationClip.js'
import { RuntimeAnimatorController } from './runtimeAnimatorController.js'
import { KVPair } from '../basicTypes.js'
import { PPtr } from './pptr.js'

export class HumanPoseMask {
    static exposedAttributes = ['word0', 'word1', 'word2']
...
export class SkeletonMaskElement {
    static exposedAttributes = ['pathHash', 'weight']
...
export class SkeletonMask {
    static exposedAttributes = ['data']

    constructor(reader) {
        let numElements = reader.readInt32()
        this.data = new Array(numElements)
        for (let i = 0; i < numElements; i++) {
            this.data[i] = new SkeletonMaskElement(reader)
        }
    }
}
...
export class ConditionConstant {
    static exposedAttributes = ['conditionMode', 'eventID', 'eventThreshold', 'exitTime']
...
export class TransitionConstant {
    ...
    constructor(reader) {
        ...
        this.conditionConstants = new Array(numConditions)
        for (let i = 0; i < numConditions; i++) {
            this.conditionConstants[i] = new ConditionConstant(reader)
        }
...
export class LeafInfoConstant {
    static exposedAttributes = ['idArray', 'indexOffset']
...
export class MotionNeighbourList {
    static exposedAttributes = ['neighbours']
    constructor(reader) {
...
export class Blend2DDataConstant {
    ...
    constructor(reader) {
        ...
        this.childNeighbourLists = new Array(numNeighbours)
        for (let i = 0; i < numNeighbours; i++) {
            this.childNeighbourLists[i] = new MotionNeighbourList(reader)
        }
    }
}

export class Blend1DDataConstant {
    static exposedAttributes = ['childThresholds']
...
export class BlendDirectDataConstant {
    static exposedAttributes = ['childBlendEventIDs', 'normalizedBlendValues']
...
export class BlendTreeNodeConstant {
    ...
    constructor(reader) {
        ...
        if (
            ...
            (reader.version[0] === 4 && reader.version[1] === 1 && reader.version[2] >= 3)
        ) {
            // 4.1.3+
...
export class BlendTreeConstant {
    static exposedAttributes = ['nodes']

    constructor(reader) {
        let numNodes = reader.readInt32()
        this.nodes = new Array(numNodes)
        for (let i = 0; i < numNodes; i++) {
            this.nodes[i] = new BlendTreeNodeConstant(reader)
        }
...
export class StateConstant {
    ...
    constructor(reader) {
        ...
        this.transitionConstants = new Array(numTransitions)
        for (let i = 0; i < numTransitions; i++) {
            this.transitionConstants[i] = new TransitionConstant(reader)
        }
        ...
        if (reader.versionLT(5, 2)) {
            ...
            this.leafInfoArray = new Array(numInfos)
            for (let i = 0; i < numInfos; i++) {
                this.leafInfoArray[i] = new LeafInfoConstant(reader)
            }
        }
        let numBlends = reader.readInt32()
        this.blendTreeConstants = new Array(numBlends)
        for (let i = 0; i < numBlends; i++) {
            this.blendTreeConstants[i] = new BlendTreeConstant(reader)
        }
...
export class SelectorTransitionConstant {
    static exposedAttributes = ['destination', 'conditionConstants']
...
export class SelectorStateConstant {
    static exposedAttributes = ['transitionConstants', 'fullPathID', 'isEntry']

    constructor(reader) {
        let numTransitions = reader.readInt32()
        this.transitionConstants = new Array(numTransitions)
        for (let i = 0; i < numTransitions; i++) {
            this.transitionConstants[i] = new SelectorTransitionConstant(reader)
        }
        this.fullPathID = reader.readUInt32()
...
export class StateMachineConstant {
    ...
    constructor(reader) {
        ...
        this.stateConstants = new Array(numStates)
        for (let i = 0; i < numStates; i++) {
            this.stateConstants[i] = new StateConstant(reader)
        }
        let numAnyStates = reader.readInt32()
        this.anyStateTransitionConstants = new Array(numAnyStates)
        for (let i = 0; i < numAnyStates; i++) {
            this.anyStateTransitionConstants[i] = new TransitionConstant(reader)
        }
        if (reader.version[0] >= 5) {
            let numSelectors = reader.readInt32()
            this.selectorStateConstants = new Array(numSelectors)
            for (let i = 0; i < numSelectors; i++) {
                this.selectorStateConstants[i] = new SelectorStateConstant(reader)
            }
        }
...
export class ValueArray {
    ...
    constructor(reader) {
        ...
        if (reader.versionLT(4, 3)) {
            ...
            this.positionValues = new Array(numPosValues)
            for (let i = 0; i < numPosValues; i++) {
                this.positionValues[i] = reader.versionGTE(5, 4) ? reader.readVector3() : reader.readVector4()
            }
            ...
            this.scaleValues = new Array(numScaleValues)
            for (let i = 0; i < numScaleValues; i++) {
                this.scaleValues[i] = reader.versionGTE(5, 4) ? reader.readVector3() : reader.readVector4()
            }
...
export class ControllerConstant {
    static exposedAttributes = ['layers', 'stateMachines', 'values', 'defaultValues']

    constructor(reader) {
        let numLayers = reader.readInt32()
        this.layers = new Array(numLayers)
        for (let i = 0; i < numLayers; i++) {
            this.layers[i] = new LayerConstant(reader)
        }

        let numStates = reader.readInt32()
        this.stateMachines = new Array(numStates)
        for (let i = 0; i < numStates; i++) {
            this.stateMachines[i] = new StateMachineConstant(reader)
        }
...
export class AnimatorController extends RuntimeAnimatorController {
    static exposedAttributes = ['controllerSize', 'controller', 'tos', 'animationClips']
    ...
    constructor(reader) {
        ...
        this.tos = new Array(numTos)
        for (let i = 0; i < numTos; i++) {
            let key = reader.readUInt32()
            this.tos[i] = new KVPair(key, reader.readAlignedString())
        }

        let numClips = reader.readInt32()
        this.animationClips = new Array(numClips)
        for (let i = 0; i < numClips; i++) {
            this.animationClips[i] = new PPtr(reader)
        }
    }
}
