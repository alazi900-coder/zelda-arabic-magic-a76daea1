...
import { AnimTransform } from './animationClip.js'
import { KVPair } from '../basicTypes.js'

export class AvatarNode {
    static exposedAttributes = ['parentID', 'axesID']
...
export class Axes {
    static exposedAttributes = ['preQ', 'postQ', 'sgn', 'limit', 'length', 'type']
...
export class Handle {
    static exposedAttributes = ['transform', 'parentHumanIndex', 'id']
    constructor(reader) {
...
export class Human {
    ...
    constructor(reader) {
        ...
        if (reader.versionLT(2018, 2)) {
            ...
            this.handles = new Array(numHandles)
            for (let i = 0; i < numHandles; i++) {
                this.handles[i] = new Handle(reader)
            }
            let numColliders = reader.readInt32()
            this.colliders = new Array(numColliders)
            for (let i = 0; i < numColliders; i++) {
                this.colliders[i] = new Collider(reader)
            }
        }
...
export class Avatar extends NamedObject {
    static exposedAttributes = ['name', 'avatarSize', 'avatar', 'tos']
    constructor(reader) {
        super(reader)
