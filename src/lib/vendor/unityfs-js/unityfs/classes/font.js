...
import { Rectf } from './sprite.js'
import { KVPair } from '../basicTypes.js'

export class CharacterRect {
    static exposedAttributes = ['index', 'uv', 'vert', 'width', 'flipped']
    constructor(reader) {
...
export class KerningValue {
    static exposedAttributes = ['pairFirst', 'pairSecond', 'second']
    constructor(reader) {
        // real names unknown
...
export class Font extends NamedObject {
    ...
    constructor(reader) {
        ...
        if (reader.versionGTE(5, 5)) {
            ...
            this.characterRects = new Array(numCharacterRects)
            for (let i = 0; i < numCharacterRects; i++) {
                this.characterRects[i] = new CharacterRect(reader)
            }
            let numKerningValues = reader.readInt32()
            this.kerningValues = new Array(numKerningValues)
            for (let i = 0; i < numKerningValues; i++) {
                this.kerningValues[i] = new KerningValue(reader)
            }
            ...
            if (reader.version[0] <= 3) {
                ...
                this.perCharacterKerning = new Array(numPerCharacterKerning)
                for (let i = 0; i < numPerCharacterKerning; i++) {
                    this.perCharacterKerning[i] = new KVPair(reader.readInt32(), reader.readFloat32())
                }
            } else {
                this.characterSpacing = reader.readInt32()
                this.characterPadding = reader.readInt32()
