...
import { SEEK_CUR } from '../../core/binaryStream.js'
import { decompressBlock } from '../../decoders/drivers/lz4.js'

const utf8Decoder = new TextDecoder('utf-8')

export class VectorParameter {
    static exposedAttributes = ['nameIndex', 'index', 'arraySize', 'type', 'dim']
...
export class MatrixParameter {
    static exposedAttributes = ['nameIndex', 'index', 'arraySize', 'type', 'rowCount']
...
export class StructParameter {
    static exposedAttributes = ['nameIndex', 'index', 'arraySize', 'structSize', 'vectorParams', 'matrixParams']
...
export class SamplerParameter {
    static exposedAttributes = ['sampler', 'bindPoint']
...
export class SerializedTextureProperty {
    static exposedAttributes = ['defaultName', 'textureDimension']
...
export class SerializedProperty {
    static exposedAttributes = ['name', 'description', 'attributes', 'type', 'flags', 'defaultValue', 'defaultTexture']
...
export class SerializedProperties {
    static exposedAttributes = ['properties']
...
export class SerializedShaderFloatValue {
    static exposedAttributes = ['name', 'value']
...
export class SerializedStencilOp {
    static exposedAttributes = ['pass', 'fail', 'zFail', 'comp']
...
export class SerializedShaderVectorValue {
    static exposedAttributes = ['name', 'x', 'y', 'z', 'w']
    constructor(reader) {
...
export class ShaderBindChannel {
    static exposedAttributes = ['source', 'target']
...
export class ParserBindChannels {
    static exposedAttributes = ['channels', 'sourceMap']
...
export class TextureParameter {
    static exposedAttributes = ['nameIndex', 'index', 'samplerIndex', 'dim']
...
export class BufferBinding {
    static exposedAttributes = ['nameIndex', 'index']
...
export class ConstantBuffer {
    static exposedAttributes = ['nameIndex', 'matrixParams', 'vectorParams', 'size']
...
export class UAVParameter {
    static exposedAttributes = ['nameIndex', 'index', 'originalIndex']
...
export class SerializedSubProgram {
    static exposedAttributes = ['blobIndex', 'channels', 'shaderHardwareTier', 'gpuProgramType', 'parameters']
    constructor(reader) {
...
export class SerializedProgram {
    ...
    constructor(reader) {
        ...
        if (version[0] > 2021 || (version[0] === 2021 && version[1] >= 2)) {
            ...
            const outerLen = reader.readInt32()
            this.playerSubPrograms = []
            for (let i = 0; i < outerLen; i++) {
                const innerLen = reader.readInt32()
                const subPrograms = []
                for (let j = 0; j < innerLen; j++) {
                    const subProg = {}
                    subProg.blobIndex = reader.readUInt32()
                    subProg.keywordIndices = reader.readArrayT(() => reader.readUInt16(), reader.readInt32())
                    reader.align(4)
                    subProg.shaderRequirements = Number(reader.readInt64())
                    subProg.gpuProgramType = reader.readInt8()
                    reader.align(4)
                    subPrograms.push(subProg)
                }
                this.playerSubPrograms.push(subPrograms)
            }

            const outerBlobLen = reader.readInt32()
            this.parameterBlobIndices = []
            for (let i = 0; i < outerBlobLen; i++) {
                const innerBlobLen = reader.readInt32()
                const blobIndices = []
                for (let j = 0; j < innerBlobLen; j++) {
                    blobIndices.push(reader.readUInt32())
                }
                this.parameterBlobIndices.push(blobIndices)
            }
...
function checkHasEditorData(reader) {
    if (!reader.versionGTE(2020, 2)) {
        return false
    }
    const start = reader.tell()
    try {
        const numEditorDataHashes = reader.readInt32()
        if (numEditorDataHashes < 0 || numEditorDataHashes > 1000) {
            return false
        }
...
export class SerializedSubShader {
    static exposedAttributes = ['passes', 'tags', 'lod']
...
export class SerializedShaderDependency {
    static exposedAttributes = ['from', 'to']
...
export class SerializedCustomEditorForRenderPipeline {
    static exposedAttributes = ['customEditorName', 'renderPipelineType']
...
export class SerializedShaderSubProgram {
    static exposedAttributes = ['version', 'programType', 'keywords']
    constructor(reader) {
        ...
        switch (this.programType) {
            ...
            case 'GLCore41':
            case 'GLCore43':
                this.code = utf8Decoder.decode(this.byteCode)
        }
        reader.align(4)
    }
}
...
export class SerializedShaderProgram {
    static exposedAttributes = ['programs']
    constructor(reader) {
        let numSubPrograms = reader.readInt32()
        ...
        this.subPrograms = []
        for (let i = 0; i < this.subProgramEntries.length; i++) {
...
export class Shader extends NamedObject {
    ...
    constructor(reader) {
        ...
        if (reader.versionGTE(5, 5)) {
            ...
            if (reader.versionGTE(2019, 3)) {
                this.offsets = reader.readArrayT(
                    () => reader.readArrayT(() => reader.readUInt32(), reader.readUInt32()),
                    reader.readUInt32(),
                )
                this.compressedLengths = reader.readArrayT(
                    () => reader.readArrayT(() => reader.readUInt32(), reader.readUInt32()),
                    reader.readUInt32(),
                )
                this.decompressedLengths = reader.readArrayT(
                    () => reader.readArrayT(() => reader.readUInt32(), reader.readUInt32()),
                    reader.readUInt32(),
                )
            } else {
            ...
            if (reader.version[0] > 2021 || (reader.version[0] === 2021 && reader.version[1] >= 2)) {
                this.stageCounts = reader.readArrayT(() => reader.readUInt32(), reader.readInt32())
            }
    ...
    async getExport() {
        ...
        for (const s of this.decompressedLengths) {
            for (const v of s) {
                uncompressedSize += v
            }
        }
        const outBuf = new Uint8Array(uncompressedSize)
        let outOffset = 0
        let di = 0
        for (const s of this.compressedLengths) {
            let d = 0
            for (const v of s) {
                const offset = this.offsets[di][d]
                const decLength = this.decompressedLengths[di][d]
                const compSlice = this.compressedBlob.subarray(offset, offset + v)
                decompressBlock(compSlice, outBuf, 0, compSlice.length, outOffset)
                outOffset += decLength
                d++
            }
            di++
