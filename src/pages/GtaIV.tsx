/** GTA IV design: a focused workbench for the Russian-slot Arabic mod's russian.gxt — its font draws Arabic on 124 measured glyph units, verified against the mod's own font texture. See gtaiv-ru-charmap.ts. */
import { useCallback, useMemo, useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { ArrowRight, CheckCircle2, FileCode2, Loader2, Search, ShieldCheck } from "lucide-react";
import { toast } from "sonner";
import { decodeGtaIvArabicFontUnits, inspectGtaIvGxt, parseGtaIvGxt, type GtaIvGxtSummary } from "@/lib/gtaiv/gxt-format";
import {
  extractGtaIvEntries,
  GTAIV_BUFFER_KEY,
  GTAIV_SOURCE_GAME,
...
type ArabicSourceRow = { table: string; crc: number; value: string };

export default function GtaIV() {
  const [gxt, setGxt] = useState<GtaIvGxtSummary | null>(null);
  const [rows, setRows] = useState<ArabicSourceRow[]>([]);
  ...
  const inspectRussianGxt = useCallback(async (file: File) => {
    setBusy(true);
    try {
      ...
      const sourceRows = parsed.tables.flatMap((table) => table.entries.map((entry) => ({
        table: table.name,
        crc: entry.crc,
        value: decodeGtaIvArabicFontUnits(entry.textUnits, true),
      })));
      ...
      toast.success(`قُرئ russian.gxt: ${summary.entries.toLocaleString("ar")} سطراً.`);
      navigate("/editor");
    } catch (error) {
    ...
    <main className="min-h-screen bg-background text-foreground" dir="rtl">
      <section className="mx-auto max-w-6xl px-4 py-10">
        <header className="mb-8 flex items-center justify-between gap-3">
          <div>
            <p className="text-sm text-primary">GTA IV · مود روسي معرَّب</p>
            <h1 className="text-2xl font-bold">مسار مود russian.gxt العربي</h1>
          </div>
        ...
        <div className="mb-6 rounded-xl border border-primary/30 bg-primary/5 p-4 text-sm text-foreground">
          <p className="flex gap-2 font-semibold"><FileCode2 className="h-5 w-5 shrink-0 text-primary" /> المسار النشط: تعديل الترجمة العربية داخل مود russian.gxt الروسي.</p>
          <p className="mt-2 text-muted-foreground">حمّل <code>russian.gxt</code> ليفتح مباشرة داخل محرر الأداة الحالي. خطّ هذا المود يرسم العربية على ١٢٤ خانة مقاسة من نسيج خطّه، لا على النص الإنجليزي الأصلي.</p>
        </div>
        ...
        <div className="flex min-h-52 flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-primary/35 bg-card p-6 text-center transition hover:border-primary">
          ...
          <strong>افتح النص العربي داخل المحرر</strong>
        ...
        {gxt && <section className="mt-6 space-y-4 rounded-xl border bg-card p-5">
          ...
          <div className="max-h-[34rem] overflow-auto rounded-lg border">
            <table className="w-full min-w-[720px] text-right text-sm">
              <thead className="sticky top-0 bg-muted text-muted-foreground"><tr><th className="p-3">الجدول</th><th className="p-3">CRC</th><th className="p-3">النص العربي</th></tr></thead>
              <tbody>{visibleRows.map((row) => <tr className="border-t border-border/60 align-top" key={`${row.table}:${row.crc}`}><td className="p-3 font-mono text-xs">{row.table}</td><td className="p-3 font-mono text-xs">0x{row.crc.toString(16).padStart(8, "0")}</td><td className="p-3" dir="rtl"><ArabicValue value={row.value} /></td></tr>)}</tbody>
            </table>
          </div>
        </section>}
        ...
        <section className="mt-6 space-y-3 rounded-xl border bg-card p-5 text-sm text-muted-foreground">
          ...
          <p>هذا القسم مخصص لمود russian.gxt الروسي فقط. يحفظ المصدر الخام محلياً، ثم يبني المحرر <code>russian.gxt</code> منفصلاً من الترجمات التي تعدلها.</p>
        </section>
      </section>
    </main>
  );
...
function ArabicValue({ value }: { value: string }) {
