...
import { ArrowRight, CheckCircle2, Download, ImageDown, Info, Loader2, Palette, Replace, RotateCcw, Upload } from "lucide-react";
      ...
      {!rom ? <section className="grid overflow-hidden rounded-2xl border border-[#78663b] bg-[#211c12] md:grid-cols-[1.2fr_0.8fr]">
        ...
        <section className="mb-6 grid gap-4 md:grid-cols-[260px_1fr]">
          ...
          <div className="rounded-2xl border border-[#6c5b32] bg-[#211c12] p-5 sm:p-7">
            ...
            <div className="mt-7 grid gap-6 lg:grid-cols-2"><div><p className="mb-3 text-xs font-bold text-[#bcb198]">الأصل من ROM</p>{original && <div className="flex min-h-40 items-center justify-center rounded-xl border border-[#625631] bg-[#0e0c09] p-6"><img src={pixelsToUrl(original)} alt="NEW GAME الأصلي" className="w-full max-w-[360px] object-contain" style={{ imageRendering: "pixelated", transform: "scale(4)", transformOrigin: "center" }} /></div>}</div><div><p className="mb-3 text-xs font-bold text-[#bcb198]">المعاينة بعد الاستبدال</p>{current && <div className="flex min-h-40 items-center justify-center rounded-xl border border-dashed border-[#827240] bg-[#17130d] p-6"><img src={pixelsToUrl(current)} alt="معاينة مورد Reshef" className="w-full max-w-[360px] object-contain" style={{ imageRendering: "pixelated", transform: "scale(4)", transformOrigin: "center" }} /></div>}</div></div>
