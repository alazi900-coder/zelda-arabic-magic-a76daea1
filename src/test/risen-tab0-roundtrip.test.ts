...
const buildTab0 = (fields: FieldSpec[], magic: "TAB0" | "TAB1" = "TAB0"): Uint8Array => {
...
/** Wrap raw TAB1 table byte blocks into a Risen-2-shaped file — each table
 * individually zlib-deflated, size1 = compressed length, size2 = raw length
 * (matches the real Risen 2 strings.p00 layout confirmed via a real file). */
function wrapAsFileCompressed(rawTables: Uint8Array[], names: string[]): ArrayBuffer {
  const HEADER_SIZE = 48;
  const reservedTrailer = new Uint8Array(32);
  const compressedTables = rawTables.map((t) => deflate(t));
  ...
  const entryBytesList = names.map((name, i) => {
    ...
    dv.setUint32(p, 0, true); p += 4; // zero1
    dv.setUint32(p, 0, true); p += 4; // zero2
    dv.setUint32(p, compressedTables[i].length, true); p += 4; // size1 (compressed)
    dv.setUint32(p, rawTables[i].length, true); p += 4; // size2 (uncompressed)
    return buf;
  });
  ...
  for (let i = 0; i < compressedTables.length; i++) out.set(compressedTables[i], tableOffsets[i]);
  out.set(reservedTrailer, dataEnd);
...
describe("Risen 2 (TAB1, zlib-compressed tables)", () => {
  function buildSyntheticCompressed(): ArrayBuffer {
    const table1 = buildTab0([
      { name: "ID", values: ["Q1", "Q2"] },
      { name: "English_Text", values: ["Hello world", "Second quest"] },
      { name: "German_Text", values: ["Hallo Welt", "Zweite Quest"] },
    ...
    return wrapAsFileCompressed([table1, table2], ["quests.tab", "infos.tab"]);
  }

  it("detects the compressed format and parses table structure through inflate", () => {
    const buffer = buildSyntheticCompressed();
    const doc = parseRisenP00Full(buffer);
    expect(doc.compressed).toBe(true);
    expect(doc.tables.length).toBe(2);
    expect(doc.tables[0].name).toBe("quests.tab");
    expect(doc.tables[0].magic).toBe("TAB1");
    expect(doc.tables[0].fields[1].values).toEqual(["Hello world", "Second quest"]);
    expect(doc.tables[1].name).toBe("infos.tab");
  });

  it("still parses an uncompressed TAB0 file as non-compressed (doesn't misdetect Risen 1 as Risen 2)", () => {
    const buffer = buildSyntheticCompressed();
    const doc = parseRisenP00Full(buffer);
    expect(doc.compressed).toBe(true);
    // Sanity check against the existing uncompressed builder from the describe block above.
    const table = buildTab0([{ name: "ID", values: ["Q1"] }, { name: "English_Text", values: ["Hi"] }]);
    const uncompressedBuffer = wrapAsFile([table], ["quests.tab"]);
    const uncompressedDoc = parseRisenP00Full(uncompressedBuffer);
    expect(uncompressedDoc.compressed).toBe(false);
  });

  it("rebuild without changes round-trips (re-parses to the exact same content, re-deflate is deterministic)", () => {
    const buffer = buildSyntheticCompressed();
    const doc = parseRisenP00Full(buffer);
    const rebuilt = buildRisenP00(doc);
    const reparsed = parseRisenP00Full(rebuilt);
  ...
  it("rebuild with Arabic overwrite changes size, recompresses, and preserves other rows/tables", () => {
    const buffer = buildSyntheticCompressed();
    const doc = parseRisenP00Full(buffer);
    const translations = new Map<string, string>();
    translations.set(makeKey("quests.tab", "English_Text", 0), "مرحبا بالعالم");
    ...
    const reparsed = parseRisenP00Full(rebuilt);
    expect(reparsed.compressed).toBe(true);
    expect(reparsed.tables[0].fields[1].values[0]).toBe("مرحبا بالعالم");
    // untouched row preserved
    expect(reparsed.tables[0].fields[1].values[1]).toBe("Second quest");
    expect(reparsed.tables[1].fields[1].values[0]).toBe("سطر حوار");
    // context field of the untouched table preserved
    expect(reparsed.tables[1].fields[2].values[0]).toBe("Vince");
  });

  it("recomputes compressed table offsets and FileInfoHdr size1/size2 correctly when a table's compressed size changes", () => {
    const buffer = buildSyntheticCompressed();
    const doc = parseRisenP00Full(buffer);
    const oldT2Offset = doc.tables[1].originalOffset;

    const translations = new Map<string, string>();
    translations.set(makeKey("quests.tab", "English_Text", 0), "Hello world" + "!".repeat(200));
    applyTranslations(doc, translations);
