/**
 * Appended to the AI prompt whenever a request is for a Risen game (1 or 2) —
 * counters the model's strong training-data pull toward Xenoblade Chronicles
 * terminology and game mechanics on generic RPG English text (stat names,
 * item types, magic systems). Confirmed real failures even when the persona
 * text already correctly said "Risen 1": the model "corrected" a Risen "Mana"
 * translation to "Ether" citing Xenoblade lore, and rewrote "Dexterity
 * affects bow damage" to "Dexterity affects hit rate" citing Xenoblade's stat
 * system. Deliberately worded to say "Risen" without committing to a specific
 * numbered entry — Risen 1 and Risen 2 have different settings/terminology of
 * their own (see RISEN_SYSTEM_PROMPT vs RISEN2_SYSTEM_PROMPT in translate-entries
 * for the per-game universe knowledge), but this guard only needs to rule out
 * Xenoblade contamination, which applies identically to both.
 */
export const RISEN_FORGET_OTHER_GAME_RULE =
  'هذه اللعبة من سلسلة Risen من Piranha Bytes — ليست Xenoblade Chronicles ولا أي جزء من سلسلتها، ولا علاقة بينهما إطلاقاً. ' +
...
export const RISEN_FORGET_OTHER_GAME_RULE_EN =
  'This is a Risen game (Piranha Bytes) — NOT Xenoblade Chronicles and unrelated to it. ' +
