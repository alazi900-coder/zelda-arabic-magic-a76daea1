...
import { mergeByIndex } from "../_shared/merge-issues.ts";
...
const RULES: RuleDef[] = [
  ...
  // 9th Dawn Remake-only rule: declared here, injected only when the
  ...
  // كان هذا المفتاح معروضاً في اللوحة ومحفوظاً ومُرسَلاً بلا أي موجّه هنا، أي
  // أن تشغيله أو إطفاءه لا يفعل شيئاً. النقطة ليست خطأً في العربية، لكن
  // إضافتها حيث لم يضعها المترجم تصرّف في أسلوبه — وفي الألعاب ذات الخانات
  // الثابتة حرف زائد قد يدفع السطر فوق سعته فيُرفض.
  { id: 'no_added_terminal_dot', kind: 'protect', prompt: '🚫 لا تُضف نقطة "." (أو "،" أو "؛") في نهاية الترجمة العربيّة إذا لم تكن موجودة أصلاً. احترم اختيار المترجم بشأن علامات الترقيم النهائيّة، حتى لو كان النصّ الإنجليزي ينتهي بنقطة.' },
...
/**
 * 9th Dawn Remake's four token shapes, matched by the same pattern the
 * client uses (ninthdawn-tags.ts) so both sides refuse exactly the same
 * suggestion.
 */
const NINTHDAWN_TOKEN_REGEX = /<\/?b>|<\/?i>|<br>|\[playername\]|\[cardgamename\]|\[[a-z]=\d+\]|\[\d+\]/g;
function preservesNinthDawnTokenSequence(original: string, candidate: string): boolean {
  const expected = (original || '').match(NINTHDAWN_TOKEN_REGEX) || [];
  const actual = (candidate || '').match(NINTHDAWN_TOKEN_REGEX) || [];
  return expected.length === actual.length && expected.every((token, index) => token === actual[index]);
}
...
async function runPasses<T extends { index?: number }>(
  ...
  return { merged: mergeByIndex(items) };
}
