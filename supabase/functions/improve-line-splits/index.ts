...
interface ReqBody {
  engine: "lovable" | "gemini-direct" | "google-translate" | "deepseek" | "tokenrouter" | "local";
  model?: string;
  apiKey?: string;
  entries: ReqEntry[];
...
async function callTokenRouter(entries: ReqEntry[], apiKey: string): Promise<Record<string, string>> {
  const prompt = buildPrompt(entries) + `\n\nأعد JSON صالحاً فقط بالشكل: {"results":[{"key":"...","text":"..."}]}`;
  ...
  if (resp.status === 429) throw new Error("429: TokenRouter rate limit");
  if (resp.status === 402) throw new Error("402: TokenRouter credits");
  if (!resp.ok) throw new Error(`TokenRouter ${resp.status}: ${(await resp.text()).slice(0, 300)}`);
  const data = await resp.json();
  const text = data?.choices?.[0]?.message?.content || "{}";
  let parsed: { results?: Array<{ key: string; text: string }> } = {};
  try { parsed = JSON.parse(text); } catch { /* try to extract */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    } else if (body.engine === "tokenrouter") {
      const key = body.apiKey || Deno.env.get("TOKENROUTER_API_KEY");
      if (!key) throw new Error("مفتاح TokenRouter مطلوب");
      raw = await callTokenRouter(promptEntries, key);
    } else {
