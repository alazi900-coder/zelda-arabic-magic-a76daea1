...
- لا تُقدّم فعلاً على فعل آخر أو تعيد ترتيب الجملة إلا إذا كان الترتيب الحالي خاطئاً نحوياً
- الإصلاح يجب أن يكون أقرب ما يمكن للنص الحالي — غيّر فقط الجزء الذي فيه مشكلة
- إذا كان النص مقبولاً ومفهوماً، لا تضعه في النتائج

أنواع المشاكل:
1. **literal** — ترجمة حرفية جامدة لا تبدو طبيعية بالعربية
2. **grammar** — خطأ نحوي أو صرفي واضح
3. **inconsistency** — مصطلح مترجم بشكل مختلف عن القاموس المعتمد
4. **naturalness** — صياغة ركيكة بشكل واضح (ليس مجرد تفضيل أسلوبي)
...
- لا تقترح تغييرات أسلوبية أو تحسينات صياغة — فقط الأخطاء
- إذا كان النص صحيحاً نحوياً وإملائياً، لا تضعه في النتائج حتى لو كانت الصياغة غير مثالية
- لا تُعِد ترتيب الجمل أو تغيّر بنيتها — فقط صحّح الخطأ المحدد
...
3. **case** — خطأ إعرابي
4. **spelling** — خطأ إملائي (مثل "لاكن" بدل "لكن")
