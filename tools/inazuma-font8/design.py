# 7x8 Arabic forms for Inazuma's FONT8, drawn with the joining line on row 5
# and then moved up a row (see `raise_one`): the name label over the dialogue
# box shows only rows 0-6 of a glyph. So in the font the joining line is row 4,
# descenders and dots below use rows 5-6, and ascenders and dots above rows 0-2.
def G(*rows):
    rows = list(rows)
...
def raise_one(g):
    # Drop the top row, or -- when a mark sits on it -- the first empty row
    # under the mark, and leave row 7 empty.
    rows = list(g)
    drop = 0 if '#' not in rows[0] else next(y for y in range(1, 8) if '#' not in rows[y])
    del rows[drop]
    return rows + ['.' * len(g[0])]

assert len(GLYPHS) == 125, len(GLYPHS)
GLYPHS = [raise_one(g) for g in GLYPHS]
for i, g in enumerate(GLYPHS):
    assert len(g) == 8 and all(len(r) == len(g[0]) <= 7 for r in g), (hex(0xFE80 + i), g)

if __name__ == "__main__":
    # Prints INAZUMA_FONT8_GLYPHS_B64 and INAZUMA_FONT8_WIDTHS for
    # src/lib/inazuma/inazuma-arabic-glyphs.ts: each glyph left-aligned in its
    # 7x8 cell, rows packed MSB-first, 7 bytes a glyph.
    import base64
    packed = bytearray()
    for g in GLYPHS:
        bits = [1 if y < 8 and x < len(g[y]) and g[y][x] == '#' else 0 for y in range(8) for x in range(7)]
        packed += bytes(int(''.join(map(str, bits[i:i + 8])), 2) for i in range(0, 56, 8))
    print(base64.b64encode(bytes(packed)).decode())
    print(', '.join(str(len(g[0])) for g in GLYPHS))
